...
def main():
    ...
    with SessionLocal() as db:
        hits = kb_svc.retrieve(db, "退货政策是什么")
    ...
    with SessionLocal() as db:
        hits = kb_svc.retrieve(db, "会员日打折吗")
    ...
    # ── 3. Eval 黄金集 ──
    r = client.post(f"{BASE}/api/admin/eval/run", headers=H, json={})
    body = r.json()
    ...
    r = client.get(f"{BASE}/api/admin/eval/runs", headers=H)
    ...
    # ── 5. 演示重置 ──
    r = client.post(f"{BASE}/api/admin/demo/reset", headers=H)
