...
def _resolve(agent: str | None) -> tuple[OpenAI, str, float | None]:
    """按 agent 名查绑定表；查不到或库不可用 → 默认供应商（永不因配置缺失挂掉）。"""
    if agent:
        try:
            ...
            with SessionLocal() as db:
                row = db.execute(
                    ...
                    .where(AgentModelBinding.agent_name == agent, ModelProvider.enabled)
                ).first()
                if row:
...
def chat(messages: list[dict], model: str | None = None,
         temperature: float | None = None, agent: str | None = None) -> str:
    """调用 LLM，返回纯文本回复。agent 决定走哪个绑定模型。"""
    client, bound_model, bound_temp = _resolve(agent)
